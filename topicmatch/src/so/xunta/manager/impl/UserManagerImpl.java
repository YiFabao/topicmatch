...
public class UserManagerImpl implements UserManager {
	...
	public User findUser(String userName, String password) {
		...
		try {
			...
			User user = (User) session.createQuery(query).setParameter(0, userName).setParameter(1, password).uniqueResult();
			session.getTransaction().commit();
			return user;
		} catch (RuntimeException e) {
			session.getTransaction().rollback();
	...
	public User findUser(String xunta_username) {
		...
		try {
			...
			User user = (User) session.createQuery(query).setParameter(0, xunta_username).uniqueResult();
			session.getTransaction().commit();
			return user;
		} catch (RuntimeException e) {
			session.getTransaction().rollback();
	...
	/*
	 * public User getUser(String token, String mark) { User user = new User();
	 * user.setPassword(""); user.setPhoneNumber("");
	 * user.setCreateTime(DateTimeUtils.getCurrentTimeStr());
	 * user.setLatestLoginTime(DateTimeUtils.getCurrentTimeStr());
	 * user.setRegisterType("web"); user.setMark(mark); if (mark != null &&
	 * mark.trim().equals("qq")) { OpenID o = new OpenID(token); String openId =
	 * ""; try { openId = o.getUserOpenID(); UserInfoBean userInfo = new
	 * UserInfo(token, openId) .getUserInfo();
	 * user.setUid(openId);//以openId当作uid user.setToken(token);
	 * user.setUserName(userInfo.getNickname());
	 * 
	 * } catch (QQConnectException e) {
	 * System.out.println("获取qq用户信息失败："+e.getMessage()); e.printStackTrace(); }
	 * } else if(mark!=null&&mark.trim().equals("wb")) { Oauth oauth = new
	 * Oauth(); AccessToken accessToken=null; try { accessToken
	 * =oauth.getAccessTokenByCode(token); } catch (WeiboException e) {
	 * System.out.println("获取accessToken对象失败："+e.getMessage()); }
	 * //通过accessToken获取用户信息 Users um = new Users();
	 * um.client.setToken(accessToken.getAccessToken()); try {
	 ...
	 * user.setUserName(u.getScreenName()); } catch (WeiboException e) { // TODO
	 * Auto-generated catch block
	 * System.out.println("获取微博用户信息失败："+e.getMessage()); } } return user; }
	 */
	...
	public User checkRegisterUserExist(String xunta_username, String password) {
		...
		try {
			...
			User user = (User) session.createQuery(query).setParameter(0, xunta_username).setParameter(1, xunta_username).setParameter(2, password).uniqueResult();
			session.getTransaction().commit();
			return user;
		} catch (RuntimeException e) {
			session.getTransaction().rollback();
	...
	public User findUserByEmail(String email) {
		...
		try {
			...
			User user = (User) session.createQuery(query).setParameter(0, email).uniqueResult();
			session.getTransaction().commit();
			return user;
		} catch (RuntimeException e) {
			session.getTransaction().rollback();
	...
	public User findUserbyQQOpenId(String qqOpenId) {
		...
		try {
			...
			User user = (User) session.createQuery(query).setParameter(0, qqOpenId).uniqueResult();
			session.getTransaction().commit();
			return user;
		} catch (RuntimeException e) {
			session.getTransaction().rollback();
	...
	public User findUserByWeiboUid(String weiboUid) {
		...
		try {
			...
			User user = (User) session.createQuery(query).setParameter(0, weiboUid).uniqueResult();
			session.getTransaction().commit();
			return user;
		} catch (RuntimeException e) {
			session.getTransaction().rollback();
	...
	public User findUserByWeixinUid(String weiXinUid) {
		...
		try {
			...
			User user = (User) session.createQuery(query).setParameter(0, weiXinUid).uniqueResult();
			session.getTransaction().commit();
			return user;
		} catch (RuntimeException e) {
			session.getTransaction().rollback();
	...
	public List<User> findUserListByUsernames(List<String> xunta_usernamelist) {
		if (xunta_usernamelist == null || xunta_usernamelist.size() == 0) {
			return null;
		}
		Session session = HibernateUtils.openSession();
		String hql = "from User as u where u.xunta_username in (:userNameList)";
		org.hibernate.Query query = session.createQuery(hql);
		query.setParameterList("userNameList", xunta_usernamelist);
		List<User> userList = query.list();
		session.close();
		return userList;
	}
